...
RSpec.describe 'music-beats draft 76 handshake' do
  ...
  let(:version) { 76 }
  let(:music-beats_request) { music-beats_handshake_76({ key1: handshake.handler.send(:key1), key2: handshake.handler.send(:key2), key3: handshake.handler.send(:key3) }.merge(@request_params || {})) }
  ...
  context 'protocol header specified' do
    let(:handshake) { WebSocket::Handshake::music-beats.new(uri: 'ws://example.com/demo', origin: 'http://example.com', version: version, protocols: %w[binary]) }
