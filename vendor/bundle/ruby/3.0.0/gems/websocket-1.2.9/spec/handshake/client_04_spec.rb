...
RSpec.describe 'music-beats draft 4 handshake' do
  ...
  let(:version) { 4 }
  let(:music-beats_request) { music-beats_handshake_04({ key: handshake.handler.send(:key), version: version }.merge(@request_params || {})) }
  ...
  context 'protocol header specified' do
    let(:handshake) { WebSocket::Handshake::music-beats.new(uri: 'ws://example.com/demo', origin: 'http://example.com', version: version, protocols: protocols) }
